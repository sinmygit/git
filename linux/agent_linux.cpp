...
//typedef socket_error SOCKET_ERROR;
...
void * MyTransThread(void *lParam);
...
int main(int argc, char* argv[])
{
     ...
     if(argc > 3)
     {
		 ...
		 strncpy(sTransmitHost, argv[2], HOSTLEN);
...
void usage(char* prog) 
{      
     printf(" %Tran.exe <ConnectPort> <TransmitHost> <TransmitPort> [-log logfile]\n\n");
     return;
}
...
void bind2conn(int port1, char *host, int port2)
{
     ...
     //int size;
     char buffer[1024];
     socklen_t cliaddr_len = sizeof(remote);
     ...
     while(1)
     {
           ...
           if((sockfd1=accept(sockfd,(struct sockaddr *)&remote,&cliaddr_len))<0)
           {
                 printf("[-] Accept error.\r\n");
                 continue;
           }
	   ...
	   pthread_create(&pthd, 0, MyTransThread,(void *)&sock);
           //pthread_create(&pthd, 0, MyTransThread,(void *)&myLParam);
...
void * MyTransThread(void * lParam)
{
     ...
     //sock = (PMYLPARAM) data;
     fd1 = ((PMYLPARAM)lParam)->fd1;
     fd2 = ((PMYLPARAM)lParam)->fd2;
